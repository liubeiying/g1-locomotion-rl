...
> 说明：上一段工作签有保密协议（NDA），前雇主的专有代码与实验结果不在公开范围，也不在本仓库里。这里放的是公开开源项目的个人复现记录，可放心公开。
...
仓库成型的部分是**复现链路与方法**：环境对照表、六个踩坑排查、奖励与课程设计、sim2sim 导出都写在 `docs/` 里，这部分可以直接看。

实测数据（训练曲线、sim2sim 一致性、真机行走）是我自己的复现实验，目前还在补录：
...
> 前雇主的专有结果与代码受 NDA 约束不在公开范围，上面三项是我自己的复现实验，不会用别人的数字替代。补数据时我会把测试条件一起写清楚——没有前提条件的数字没有意义。
